...
"""Starter script for blog-db-manage."""
...
from blog.openstack.common import importutils
from blog.services import action_manager
# from blog.services import workflows


# We need to import blog.api.app to
# make sure we register all needed options.
importutils.try_import('blog.api.app')
...
def main():
    ...
    config.set_main_option(
        'script_location',
        'blog.db.sqlalchemy.migration:alembic_migrations'
    )
    # attach the Blog conf to the Alembic conf
    config.blog_config = CONF
