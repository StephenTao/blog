...
class _BlogModelBase(oslo_models.ModelBase, oslo_models.TimestampMixin):
    """Base class for all Blog SQLAlchemy DB Models."""
    ...
    def to_dict(self):
        ...
        datetime_to_str(d, 'created_time')
        datetime_to_str(d, 'updated_time')
    ...
    def get_clone(self):
        ...
        setattr(m, 'created_time', getattr(self, 'created_time').isoformat(' '))
        setattr(m, 'updated_time', getattr(self, 'updated_time').isoformat(' '))
...
BlogModelBase = declarative.declarative_base(cls=_BlogModelBase)


# Secure model related stuff.


class BlogSecureModelBase(BlogModelBase):
...
def register_secure_model_hooks():
    ...
    for sec_model_class in utils.iter_subclasses(BlogSecureModelBase):
