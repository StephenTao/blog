...
from blog.actions import base
from blog import exceptions as exc
from blog.openstack.common import log as logging
from blog.utils import javascript
from blog.utils import ssh_utils
...
class BlogHTTPAction(HTTPAction):
    def __init__(self,
                 action_context,
                 url,
                 method="GET",
        ...
        headers = headers or {}
        headers.update({
            'Blog-Workflow-Name': actx.get('workflow_name'),
            'Blog-Workflow-Execution-Id': actx.get('workflow_execution_id'),
            'Blog-Task-Id': actx.get('task_id'),
            'Blog-Action-Execution-Id': actx.get('action_execution_id'),
        })

        super(BlogHTTPAction, self).__init__(
            url,
            method,
            params,
            body,
