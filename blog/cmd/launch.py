...
# If ../blog/__init__.py exists, add ../ to Python search path, so that
# it will override what happens to be installed in /usr/(local/)lib/python...
POSSIBLE_TOPDIR = os.path.normpath(os.path.join(os.path.abspath(sys.argv[0]),
                                   os.pardir,
                                   os.pardir))
if os.path.exists(os.path.join(POSSIBLE_TOPDIR, 'blog', '__init__.py')):
...
from blog import context as ctx
from blog.db.v1 import api as db_api
from blog.engine import default_engine as def_eng
from blog.engine import default_executor as def_executor
from blog.engine import rpc
from blog.openstack.common import log as logging
from blog.services import scheduler
...
def launch_api(transport):
    ...
    LOG.info("Blog API is serving on http://%s:%s (PID=%s)" %
             (host, port, os.getpid()))
